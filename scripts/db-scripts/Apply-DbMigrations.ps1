...
$env_vars_backup = Get-MultipleEnvVariables -Variables $required_env_variables
...
if ($env_type -eq "local") {
    ...
    Invoke-Expression "`$env:ASPNETCORE_ENVIRONMENT=`"$($target_cloud_envs[$target_cloud_env])`""
        
    . "$PSScriptRoot\..\setup-scripts\SetUp-EnvironmentVariables.ps1"
}
